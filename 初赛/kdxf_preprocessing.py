...
test_files = os.listdir(datapath+'test1')

#预处理时间序列的异常偏移
##偏移校正后会生成新的文件存储在根目录下的train_new和test1_new文件夹下
get_together0(12,train_files,preprocess_single_sample,'train')
print('train时间偏移处理完毕！')
get_together0(12,test_files,preprocess_single_sample,'test1')
print('test时间偏移处理完毕！')

################################# 构造样本 ###################################

train_perlist = [0.45,0.55,0.63,0.75,0.85]
test_perlist = [1]
train = get_together(16,train_files,train_perlist,process_sample_single,'train_new',keep_per = True)
test = get_together(16,test_files,test_perlist,process_sample_single,'test1_new',keep_per = True)
